...
rm -f "$LOG"; (cd "$CONTROL_DIR"&&TWINDER_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" "PYTHON=$FAKE_PYTHON" check) >/dev/null 2>&1; grep -Fq "$FAKE_PYTHON" "$LOG"; [ ! -e "$CONTROL_DIR/TWINDER_ROOT_MARKER" ]; [ ! -e "$CONTROL_DIR/TWINDER_PYTHON_MARKER" ]
...
build check contract-test lint root-test test verify:
	@/usr/bin/touch "$$TWINDER_LATER_MARKER"
LATER_MAKE
for target in build check contract-test lint root-test test verify; do
  rm -f "$LATER_MARKER"
  if (cd "$CONTROL_DIR"&&TWINDER_LATER_MARKER="$LATER_MARKER" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER" "$target" "PYTHON=$FAKE_PYTHON") >"$TEMP_ROOT/later-$target" 2>&1; then exit 1; fi
...
#!/bin/sh
/usr/bin/touch "$TWINDER_LATER_ROOT_MARKER"
SCRIPT
...
build check contract-test lint root-test test verify: PYTHON := /usr/bin/true
LATER_VARS_MAKE
rm -f "$LOG" "$LATER_ROOT_MARKER"
if ! (cd "$CONTROL_DIR"&&TWINDER_COMMAND_LOG="$LOG" TWINDER_LATER_ROOT_MARKER="$LATER_ROOT_MARKER" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_VARS" root-test "PYTHON=$FAKE_PYTHON") >"$TEMP_ROOT/later-vars" 2>&1; then cat "$TEMP_ROOT/later-vars" >&2; exit 1; fi
grep -Fq "$CHECKOUT" "$LOG"
[ ! -e "$LATER_ROOT_MARKER" ]
rm -f "$LOG"
if ! (cd "$CONTROL_DIR"&&TWINDER_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_VARS" lint "PYTHON=$FAKE_PYTHON") >"$TEMP_ROOT/later-python" 2>&1; then cat "$TEMP_ROOT/later-python" >&2; exit 1; fi
...
if ! (cd "$CONTROL_DIR"&&TWINDER_COMMAND_LOG="$LOG" TWINDER_LATER_SHELL_LOG="$LATER_SHELL_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_SHELL" check "PYTHON=$FAKE_PYTHON") >"$TEMP_ROOT/later-shell" 2>&1; then cat "$TEMP_ROOT/later-shell" >&2; exit 1; fi
grep -Fq "$CHECKOUT" "$LOG"
[ ! -e "$LATER_SHELL_LOG" ]
LATER_OVERRIDE="$TEMP_ROOT/later-override.mk"
cat >"$LATER_OVERRIDE" <<LATER_OVERRIDE_MAKE
build check contract-test lint root-test test verify: MAKEFILE_LIST := $MAKEFILE
build check contract-test lint root-test test verify: override SHELL := $LATER_FAKE_SHELL
build check contract-test lint root-test test verify: override .SHELLFLAGS := -c
LATER_OVERRIDE_MAKE
rm -f "$LATER_SHELL_LOG"
(cd "$CONTROL_DIR"&&TWINDER_LATER_SHELL_LOG="$LATER_SHELL_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" -f "$LATER_OVERRIDE" check "PYTHON=$FAKE_PYTHON") >"$TEMP_ROOT/later-override" 2>&1
[ -s "$LATER_SHELL_LOG" ]
...
chmod +x "$PATH_XCODE"
rm -f "$PATH_XCODE_LOG"
(cd "$CONTROL_DIR"&&PATH="$TEMP_ROOT:/usr/bin:/bin" TWINDER_PATH_XCODE_LOG="$PATH_XCODE_LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" build "PYTHON=$FAKE_PYTHON") >"$TEMP_ROOT/path-xcode" 2>&1
[ ! -e "$PATH_XCODE_LOG" ]
